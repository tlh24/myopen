...
% $Revision: 1.4 $  $Date: 2013/01/27 15:05:53 $
    ...
    try
        ...
        if isempty(paramsStruct.handle)
            ...
            if isempty(paramsStruct.position)
                paramsStruct.position = getInteractivePosition(paramsStruct.hFigure);
...
function [positionRect, jFrameUsed] = getInteractivePosition(hFig)
    try
    ...
    if prod(positionRect(3:4)) > 0
        uiwait(msgbox(sprintf('%dx%d area captured',positionRect(3),positionRect(4)), 'ScreenCapture'));
    end
