...
Headstage::Headstage(int *channel,  const std::vector<Channel*> &c){
// for 31.25ksps -- see filter_butter.m
// broken into 2 biquads, order b0 b1 a0 a1
  
  ptr_c = c; //shallow copy because the containers are pointers
  ptr_channel = channel;
...
void Headstage::saveMessage(const char *fmt, ...){
	...
	for(int tid = 0; tid < NSCALE; tid++){
			int e = echo[tid] % 16;
			messages[messW[tid] % 1024][0] = 'A'+e;
			messages[messW[tid] % 1024][1] = ' ';
			vsnprintf(messages[messW[tid] % 1024] + 2, 126, fmt, ap);
			va_end(ap);
			messW[tid]++;
	}
}
void Headstage::updateGain(int chan){
	...
	*/
	...
	chan = chan & (0xff ^ 32); //map to the lower channels. Should still map properly to lower channels if > 128
		// e.g. 42 -> 10,42; 67 -> 67,99 ; 100 -> 68,100. This should be usef if chan is indexing ptr_c
	float gain1 = sqrt(fabs(ptr_c[chan]->m_gain));
	float gain2 = sqrt(fabs(ptr_c[chan+32]->m_gain));
	...
	if(ptr_c[chan]->m_gain < 0){
		b[0] *= -1; b[2] *= -1; //change the sign of the first filter.
	}
	if(ptr_c[chan+32]->m_gain < 0){
		b[1] *= -1; b[3] *= -1;
	}
	...
	unsigned int* ptr = sendbuf[tid];
	ptr += (sendW[tid] % sendL[tid]) * 8; //8 because we send 8 32-bit ints /pkt.
	
	int kchan = chan &127; //(to send in buf, needs to keep correct channel name) (0-127)
	//use kchan when not indexing ptr_c, thus, bitwise AND for 127 maps channels > 128 to 0-127.
	for(i=0; i<4; i++){
		...
		ptr[i*2+0] = htonl(echoHeadstage(echo[tid], A1 +
			(A1_STRIDE*(kchan & 31) +
...
void Headstage::setChans(int signalChain){
	...
	for(i=0; i<4;i++){
		...
		ptr[i*2+0] = htonl(echoHeadstage(echo[tid], (FP_BASE - FP_TXCHAN0 + 4*i)));
	...
	for(i=0; i<4; i++){
		saveMessage("chan %c %d", 'A'+i, ptr_channel[i]);
		printf("chan %c %d", 'A'+i, ptr_channel[i]);
		//save it in the sqlite db.
		//sqliteSetValue(i,"channel", (float)ptr_channel[i]);
	}
	for(int tid = 0; tid < NSCALE; tid++){
			sendW[tid]++;
			echo[tid]++;
...
void Headstage::setAGC(int ch1, int ch2, int ch3, int ch4){
	...
	for(int i=0; i<4; i++){
		...
		//ptr_c[chan]->m_agc = target; set ACG elsewhere.
		...
		int j = (int)(sqrt(32768 * ptr_c[chan]->m_agc));
		int k = (int)(sqrt(32768 * ptr_c[chan+32]->m_agc));
	...
	saveMessage("agc %d %d %d %d : %d %d %d %d ", ch1,ch2,ch3,ch4,
		(int)ptr_c[ch1]->m_agc,(int)ptr_c[ch2]->m_agc,(int)ptr_c[ch3]->m_agc,(int)ptr_c[ch4]->m_agc);
}
/*void enableAGC(int* chs, int en){
	...
	sendW[tid]++;
	saveMessage("m_agc_en %d %d %d %d : %d", chs[0], chs[1], chs[2], chs[3], en);
	echo[tid]++;
}*/
void Headstage::setAperture(int ch){
	...
	ch &= 31;
	unsigned int* ptr = sendbuf[tid];
	ptr += (sendW[tid] % sendL[tid]) * 8;
	
	for(int i=0; i<4; i++){
		ptr[i*2+0] = htonl(echoHeadstage(echo[tid], A1 +
			(A1_STRIDE*ch + (A1_TEMPLATE+A1_APERTURE)*(i/2) +
			 A1_APERTUREA + (i&1))*4));
		unsigned int u = (ptr_c[ch+64*(i&1)+128*tid]->getAperture(i/2) & 0xffff) |
						    ((ptr_c[ch+32+64*(i&1)+128*tid]->getAperture(i/2) & 0xffff)<<16);
		ptr[i*2+1] = htonl(u);
	}
	sendW[tid]++;
	...
	for(int i=0; i<4; i++){
		saveMessage("aperture %d %d,%d", ch + 32*i,
				ptr_c[ch + 32*i + 128*tid]->getAperture(0),
				ptr_c[ch + 32*i + 128*tid]->getAperture(0));
	}
	echo[tid]++;
}
void Headstage::setLMS(bool on){
	...
	{
		unsigned int* ptr = sendbuf[tid];
		ptr += (sendW[tid] % sendL[tid]) * 8; //8 because we send 8 32-bit ints /pkt. (32 byte packets)
		
		for(int i=0; i<4; i++){
			ptr[i*2+0] = htonl(echoHeadstage(echo[tid], FP_BASE - FP_WEIGHTDECAY)); //frame pointer
...
void Headstage::setTemplate(int ch, int aB){
	...
	for(int p=0; p<4; p++){
		...
		for(int i=0; i<4; i++){
			...
			a = (unsigned char)round((ptr_c[ch +  0 + 128*tid]->m_template[aB][n]+0.5f) * 255.f);
			b = (unsigned char)round((ptr_c[ch + 32 + 128*tid]->m_template[aB][n]+0.5f) * 255.f);
			c = (unsigned char)round((ptr_c[ch + 64 + 128*tid]->m_template[aB][n]+0.5f) * 255.f);
			d = (unsigned char)round((ptr_c[ch + 96 + 128*tid]->m_template[aB][n]+0.5f) * 255.f);
...
void Headstage::setBiquad(int chan, float* biquad, int biquadNum){
	...
	for(i=0; i<4; i++){
		unsigned int p = 0;
		if(kchan >= 64) p += 1; //chs 64-127 pocessed following 0-63.
		ptr[i*2+0] = htonl(echoHeadstage(echo[tid], A1 + ((kchan & 31)*A1_STRIDE +
