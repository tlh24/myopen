...
int	g_radioChannel[NSCALE] = {114}; //the radio channel to use. scale factor, how many systems are being used (128*1, etc) Need to assign this programatically, but this will do for now.
...
expose1 (GtkWidget *da, GdkEventExpose*, gpointer )
{
		...
		for(int k=0; k<2; k++){
		    for (int t=0; t<NSCALE; k++){
			  if(g_sbufR[k] < g_sbufW[k]){
				  i64 len = sizeof(g_sbuf[k][t])/8; //total # of pts.
				  i64 w = g_sbufW[k];
				  i64 sta = g_sbufR[k] % len;
				  i64 fin = w % len;
				  if(fin < sta) { //wrap
					  copyData(g_vbo2[k], sta, len, g_sbuf[k][t], 2);
					  copyData(g_vbo2[k],   0, fin, g_sbuf[k][t], 2);
				  } else {
					  copyData(g_vbo2[k], sta, fin, g_sbuf[k][t], 2);
				  }
				  g_sbufR[k] = w;
			  }
		    }
	...
	if(g_mode == MODE_RASTERS){
		...
		for(int k=0; k<2; k++){
			...
			glPointSize(2.0);
			glDrawArrays(GL_POINTS, 0, sizeof(g_sbuf[k][t])/8);
			glBindBufferARB(GL_ARRAY_BUFFER_ARB, 0);
			}
		}
...
configure1 (GtkWidget *da, GdkEventConfigure *, gpointer)
{
	...
	if(!g_vbo1Init){ //start it up!
		...
		for(int k=0; k<2; k++){
			...
			glBindBufferARB(GL_ARRAY_BUFFER_ARB, g_vbo2[k]);
			glBufferDataARB(GL_ARRAY_BUFFER_ARB, sizeof(g_sbuf[k][t]),
				0, GL_DYNAMIC_DRAW_ARB);
			glBufferSubDataARB(GL_ARRAY_BUFFER_ARB,
				0, sizeof(g_sbuf[k][t]), g_sbuf[k][t]);
			}
		}
		for(int i=0; i<128*NSCALE; i++){
...
void* sock_thread(void* param){
	...
	while(!destName[0]){
		...
		if(fromlen > 0 && n > 0){
			...
			inet_ntop(AF_INET, (const void*)(&(from.sin_addr)),
						 destName, 256);
			printf("a bridge suddenly appears at %s\n",destName);
			//send a response.
	...
	g_txsock[tid] = connect_socket(4342+g_radioChannel[tid],destName,0); //one port is not Ok
	if(!g_txsock[tid]) printf("failed to connect to bridge. bridge faints.\n");
	//default txsockAddr
				...
				for(int j=0; j<32; j++){
					...
					for(int k=0; k<2; k++){
						if(match[j] == k+1){
							...
							i64 w = g_sbufW[k] % (i64)(sizeof(g_sbuf[k][tid])/8);
							g_sbuf[k][tid][w*2+0] = (float)(time);
							g_sbuf[k][tid][w*2+1] = (float)adr;
							g_sbufW[k] ++;
...
int main(int argn, char **argc)
{
	...
	for(int i=0; i<4; i++){
		g_channel[i] = sqliteGetValue(i, "channel", i*32*NSCALE);
	}
