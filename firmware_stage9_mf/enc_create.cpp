...
int map(int in){
	//input [96B 96A 32B 32A 64B 64A 0B 0A] --
	//if template A is matched, zero template B on same channel.
	//this so we can compress a bit!
	if(in & 0x1) in &= (0xff ^ 0x2);
	if(in & 0x4) in &= (0xff ^ 0x8);
	if(in & 0x10) in &= (0xff ^ 0x20);
	if(in & 0x40) in &= (0xff ^ 0x80);
	return in;
}
...
int main(void){
	...
	// there are 128 slots; we use 81 w/ full compression.
	// really should use all slots to maximize spike detection.
	// this corresponds to 47 extra slots; only compress every other codeword.
	code = 0x2a; //start here so we have many transitions for radio
	// (usually no spikes).  0b0010 1010
	ncodes = 0;
	for(in = 0; in < 256; in++){
		lut[in] = 0;
		int m = in;
		if((in % 11) < 8) m = map(in);
		//compress 8/11ths of the inputs. This results in exactly 128 codes.
		if(m < in) lut[in] = lut[m]; //if compressed, store it.
		else{
			lut[in] = code;
			code++;
			code &= 0x7f; //wrap around.
			ncodes++;
		}
	}
