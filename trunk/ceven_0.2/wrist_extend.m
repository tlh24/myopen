...
%WRIST_EXTEND is a function that only works in conjunction with the virtual arm system.
%
%   limits = WRIST_EXTEND(limits,deg) where deg is a number performs a wrist extension   
%   on the virtual arm by a degree amount given as input.  Inputting a negative angle will  
%   perform a negative extension (flexion).  
%
%   limits = WRIST_EXTEND(limits,deg) where deg is an ASCII character performs a wrist   
...
% 
%   limits = WRIST_EXTEND(limits) performs a wrist extension on the virtual arm by a
