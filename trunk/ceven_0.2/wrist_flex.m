function limits = wrist_flex(limits,deg)
%WRIST_FLEX is a function that only works in conjunction with the virtual arm system.
%
%   limits = WRIST_FLEX(limits,deg) where deg is a number performs a wrist flexion   
%   on the virtual arm by a degree amount given as input.  Inputting a negative angle will  
%   perform a negative flexion (extension).  
%
%   limits = WRIST_FLEX(limits,deg) where deg is an ASCII character performs a wrist   
...
% 
%   limits = WRIST_FLEX(limits) performs a wrist flexion on the virtual arm by a
...
lower_lim = limits(1).wrist_fe;         % get preset lower limit of movement
upper_lim = limits(2).wrist_fe;         % get preset upper limit of movement
current_pos = limits(3).wrist_fe;       % get current position of movement
...
limits(3).wrist_fe = current_pos+deg;   % update and store new current position
