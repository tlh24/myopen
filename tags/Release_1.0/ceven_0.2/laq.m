...
global list viewSigs color g_die
g_die = 0; 
...
global viewSigs udp_timer g_die
...
if(exist('ai','var'))
	...
	UDP_bleed(); 
    udp_timer = timer('Period', 0.032, 'TimerFcn', {@plotdata, handles},'ExecutionMode','fixedRate'); 	
end
...
global list viewSigs udp_timer g_die
    ...
    if get(handles.hTDomain,'Value')                                    % plot time-domain if Time radio button is valued, else frequency domain
        ...
        switch choice                                                   % choose case based on a/d card
            ...
            case 'myopen'
				if(g_die == 0)
					print('UDP_receive');
					data = UDP_receive(32, 0);                              % get block of data from a/d
					DATA = fft(data(:,ch),fft_sz);                          % take FFT of block of data
					Pyy = DATA.* conj(DATA) / fft_sz;                       % find power
					f = fs*(0:(fft_sz/2))/fft_sz;                           % create corresponding frequency array
				end
