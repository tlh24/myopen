function limits = shoulder_pronate(limits,deg)
%SHOULDER_PRONATE is a function that only works in conjunction with the virtual arm system.
%
%   limits = SHOULDER_PRONATE(limits,deg) where deg is a number performs a shoulder pronation   
%   on the virtual arm by a degree amount given as input.  Inputting a negative angle will  
%   perform a negative pronation (supination).  
%
%   limits = SHOULDER_PRONATE(limits,deg) where deg is an ASCII character performs a shoulder   
%   pronation on the virtual arm by an amount that is the preset fraction of full range scaled 
%   by the ASCII-converted number (as a percentage).   
% 
%   limits = SHOULDER_PRONATE(limits) performs a shoulder pronation on the virtual arm by a
...
lower_lim = limits(1).shoulder_ps;      % get preset lower limit of movement
upper_lim = limits(2).shoulder_ps;      % get preset upper limit of movement
current_pos = limits(3).shoulder_ps;    % get current position of movement
...
limits(3).shoulder_ps = current_pos+deg;% update and store new current position
