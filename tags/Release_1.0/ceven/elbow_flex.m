function limits = elbow_flex(limits,deg)
%ELBOW_FLEX is a function that only works in conjunction with the virtual arm system.
%
%   limits = ELBOW_FLEX(limits,deg) where deg is a number performs an elbow flexion   
%   on the virtual arm by a degree amount given as input.  Inputting a negative angle will  
%   perform a negative flexion (extension).  
%
%   limits = ELBOW_FLEX(limits,deg) where deg is an ASCII character performs an elbow   
...
% 
%   limits = ELBOW_FLEX(limits) performs an elbow flexion on the virtual arm by a
