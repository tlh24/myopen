function limits = hand_close_power(limits,deg)
%HAND_CLOSE_POWER is a function that only works in conjunction with the virtual arm system.
%
%   limits = HAND_CLOSE_POWER(limits,deg) where deg is a number closes the hand of the virtual  
%   arm from a power grip by a degree amount given as input.  Inputting a negative angle will  
%   perform a negative close (open).  
%
%   limits = HAND_CLOSE_POWER(limits,deg) where deg is an ASCII character closes the hand of the   
...
% 
%   limits = HAND_CLOSE_POWER(limits) closes the hand of the virtaul arm from a power grip 
...
switch testing                                                          % pick test type (0 for no test, 1 for clothespin ...)
    case 1                                                              % case 1 is clothespin test
        ...
        if cur_pg > 38                                                  % if hand is wanting to close past 38 degrees 
            ...
            deg = cur_pg - current_pos;                                 % limit deg amount to move to max (38 deg)
