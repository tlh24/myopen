function limits = hand_open_key(limits,deg)
%HAND_OPEN_KEY is a function that only works in conjunction with the virtual arm system.
%
%   limits = HAND_OPEN_KEY(limits,deg) where deg is a number opens the hand of the virtual  
%   arm from a key grip by a degree amount given as input.  Inputting a negative angle will  
%   perform a negative open (close).  
%
%   limits = HAND_OPEN_KEY(limits,deg) where deg is an ASCII character opens the hand of the   
...
% 
%   limits = HAND_OPEN_KEY(limits) opens the hand of the virtaul arm from a key grip 
