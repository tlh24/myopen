function limits = hand_fingers_spread(limits,deg)
%HAND_FINGERS_SPREAD is a function that only works in conjunction with the virtual arm system.
%
%   limits = HAND_FINGERS_SPREAD(limits,deg) where deg is a number spreads the fingers of the virtual  
%   arm to a finger-spread by a degree amount given as input.  Inputting a negative angle will  
%   perform a negative spread (close).  
%
%   limits = HAND_FINGERS_SPREAD(limits,deg) where deg is an ASCII character spreads the fingers of the   
%   virtual arm to a finger-spread by an amount that is the preset fraction of full range scaled 
%   by the ASCII-converted number (as a percentage).   
% 
%   limits = HAND_FINGERS_SPREAD(limits) spreads the fingers of the virtaul arm to a finger-spread 
