...
%SHOULDER_EXTEND is a function that only works in conjunction with the virtual arm system.
%
%   limits = SHOULDER_EXTEND(limits,deg) where deg is a number performs a shoulder extension   
%   on the virtual arm by a degree amount given as input.  Inputting a negative angle will  
%   perform a negative extension (flexion).  
%
%   limits = SHOULDER_EXTEND(limits,deg) where deg is an ASCII character performs a shoulder   
...
% 
%   limits = SHOULDER_EXTEND(limits) performs a shoulder extension on the virtual arm by a
