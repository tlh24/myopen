...
class MatStateStor {
	...
	map<string, vector<float> > m_dat1; //one dimensional
	map<string, array2> m_dat2; //two dimensional
	map<string, array3> m_dat3; //three!
public:
	MatStateStor(const char* fname){
		m_name = string(fname); 
			...
			while(var != NULL){
				...
				if(var->rank == 3){
					...
					if(var->data_size == 4){
						...
						cout << "unkown data size for matlab variable " << nam << endl; 
					}
...
#define max(a,b) (a > b ? a : b)
	void setValue2(int ch, int un, const char* name, float val){
		...
		if(it != m_dat2.end()){
			...
			if(ch+1 > dims[0] || un+1 > dims[1])
				r->resize(boost::extents[max(ch+1,dims[0])][max(un+1,dims[1])]); 
			(*r)[ch][un] = val; 
		} else {
	...
	void setValue3(int ch, int un, const char* name, float* val, int siz){
		...
		if(it != m_dat3.end()){
			array3 *r = &((*it).second); 
			size_t* dims = (size_t*)r->shape(); 
			if(ch+1 > dims[0] || un+1 > dims[1] || siz > dims[2])
				r->resize(boost::extents[max(ch+1,dims[0])][max(un+1,dims[1])][max(siz,dims[2])]); 
			for(int i=0; i<siz; i++)
				(*r)[ch][un][i] = *val++; 
		} else {
	...
	void getValue3(int ch, int un, const char* name, float* val, int siz){
		...
		if(it != m_dat3.end()){
			array3 *r = &((*it).second); 
			size_t* dims = (size_t*)r->shape(); 
			if(ch < dims[0] && un < dims[1] && siz <= dims[2]){
				for(int i=0; i<siz; i++){
					*val++ = (*r)[ch][un][i]; 
				}
			}
...
#ifdef TEST
//compile with: 
// g++ -g -std=c++11 -DTEST stateMat2.cpp -lmatio -lhdf5
int main(void){
	MatStateStor ms("test.mat"); 
	if(1){
		int err = 0; 
